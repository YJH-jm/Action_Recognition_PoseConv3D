...
def parse_args():
    ...
    parser.add_argument(
        ...
        # default='configs/msg3d/msg3d_pyskl_ntu60_xsub_hrnet/j.py',
    ...
    parser.add_argument(
        ...
        default=('https://download.openmmlab.com/mmaction/pyskl/ckpt/'
                ...
                #  'msg3d/msg3d_pyskl_ntu60_xsub_hrnet/j.pth'),
...
def frame_extraction(video_path, short_side): 
    ...
    short_side = 480 default.. 
    Args:
        video_path (str): The video_path.
    """
    # Load the video, extract frames into ./tmp/video_name
    # os.path.basename() : 파일 이름만 출력
    # os.path.splitext() : 확장자만 분리 
    ...
    while flag:
        if new_h is None:
            h, w, _ = frame.shape # 기존 영상의 h와 w (w, h) = 1920 1080
            new_w, new_h = mmcv.rescale_size((w, h), (short_side, np.Inf)) # 853 480
            print(new_w, new_h)


        frame = mmcv.imresize(frame, (new_w, new_h)) # frame shape :  480, 853, 3
...
def detection_inference(args, frame_paths):
    ...
    for frame_path in frame_paths:
        result = inference_detector(model, frame_path) # result : list 
...
def pose_inference(args, frame_paths, det_results):
    ...
    for f, d in zip(frame_paths, det_results):
        ...
        pose = inference_top_down_pose_model(model, f, d, format='xyxy')[0] #  [{"bbox : ", "keypoints" : }, {"bbox" : }]
        ret.append(pose)
        prog_bar.update()
    return ret
...
def main():
    args = parse_args()
    frame_paths, original_frames = frame_extraction(args.video, # demo/ntu_sample.avi 분석할 영상
                                                    args.short_side) # 480
    ...
    # config.model.type : Recognizer3D
    ...
    label_map = [x.strip() for x in open(args.label_map, 'r', encoding="UTF-8").readlines()] # Action Label Map 

    # Get Human detection results
    det_results = detection_inference(args, frame_paths) # [numpy.ndarray_frame1, numpy.ndarray_frame2, ... ]
    torch.cuda.empty_cache()

    pose_results = pose_inference(args, frame_paths, det_results)  # [[{"bbox : ", "keypoints" : }, {"bbox" : }], frame2, .... ]
    torch.cuda.empty_cache()
    ...
    # print(args.pose_config) # 0sdemo/hrnet_w32_coco_256x192.py
