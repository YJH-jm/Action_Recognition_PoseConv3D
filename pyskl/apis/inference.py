...
def inference_recognizer(model, video, outputs=None, as_tensor=True, **kwargs):
    ...
    # cfg.data.test.pipeline[0].clip_len = 1
    print()
    test_pipeline = Compose(test_pipeline)
    data = test_pipeline(data)
    data = collate([data], samples_per_gpu=1) # type(data) : dict
    # img = data['imgs']
    # print(img.shape) # torch.Size([1, 20, 17, 48, 64, 64])
    # print(data['imgs'].shape) # torch.Size([1, 2, 17, 48, 64, 64])  # ([_, _, joint, clip_len, 64, 64]) 
